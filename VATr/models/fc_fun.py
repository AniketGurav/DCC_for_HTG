...
def read_file(file_path='your.dict'):
...
def soft_feat_compress_tau_a(n, feature, label, num_classes, gamma, wid_count_dict):
    ...
    for j in label.tolist():
        wid_index = int(j)
        wid = train_wid[wid_index]
        # wid_batch_list.append(wid)
...
def soft_feat_compress_tau_a(n, feature, label, num_classes, gamma, wid_count_dict):
    """
    @ym:
    the version of sfc_a will cause model cllopse.  <sfc + fl>
